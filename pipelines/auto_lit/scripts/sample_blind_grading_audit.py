...
With --per-criterion, writes a simple pack:
  HOW_TO_GRADE.md, index.csv, scores.csv (fill human_score),
  worksheets/audit_XXX.md (rubric guide per paper), rubric_guides/,
  and llm_grades_answer_key.json (do not open while grading).
"""
...
def blind_csv_fieldnames(
    ...
    *,
    ...
    del per_criterion, rubric_spec  # legacy wide sheet removed for per-criterion packs
...
def _criterion_defs(rubric: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    """Map criterion_id -> criterion dict plus axis_id / axis_label."""
    out: Dict[str, Dict[str, Any]] = {}
    for ax in rubric.get("axes") or []:
        if not isinstance(ax, dict):
            continue
        axis_id = str(ax.get("id") or "").strip()
        axis_label = str(ax.get("label") or axis_id).strip()
...
def render_paper_worksheet(
    sample_id: str,
    rec: PaperRecord,
    rubric: Dict[str, Any],
    applicable: Sequence[str],
) -> str:
    defs = _criterion_defs(rubric)
    role = (rec.paper_role or "").strip().lower()
    role_label = "query / microbe" if role in ("query", "microbe") else "target / host"
    lines: List[str] = [
        f"# {sample_id}",
        "",
        f"- **DOI**: `{rec.doi}`",
        f"- **Paper role**: {rec.paper_role} ({role_label} rubric)",
        f"- **Gene focus**: `{rec.gene_focus_id}`"
        + (f" ({rec.gene_focus_symbol})" if rec.gene_focus_symbol else ""),
        f"- **Alignment**: `{rec.alignment_id}`",
        f"- **Search terms**: {_md_escape(rec.gene_focus_search_terms) or rec.gene_focus_id}",
        f"- **Paper text**: `{rec.text_path}`",
        "",
        "## How to score",
        "",
        "1. Open the paper text path above and search for the gene terms.",
        "2. For each criterion below, choose **0**, **1**, or **2** using the score definitions.",
        f"3. Enter that value in `scores.csv` for this `sample_id` (`{sample_id}`) "
        "in the `human_score` column. Optional short note in `human_note`.",
        "4. Paper-level comments go in `index.csv` → `human_notes`.",
        "",
        "Scale: **0** = absent, **1** = partial/inferred, **2** = direct.",
        "",
    ]
    ...
    if scale:
        ...
        for key in sorted(scale.keys()):
            lines.append(f"- **{key}**: {_md_escape(str(scale[key]))}")
        lines.append("")

    by_axis: Dict[str, List[str]] = {}
    axis_labels: Dict[str, str] = {}
    for crit_id in applicable:
        d = defs.get(crit_id)
        if not d:
            continue
        axis_id = str(d.get("axis_id") or "")
        axis_labels[axis_id] = str(d.get("axis_label") or axis_id)
        by_axis.setdefault(axis_id, []).append(crit_id)

    lines.append("## Criteria (fill these only)")
    lines.append("")
    for axis_id, crit_ids in by_axis.items():
        lines.append(f"### {axis_labels.get(axis_id, axis_id)}")
        lines.append("")
        for crit_id in crit_ids:
            d = defs[crit_id]
            label = str(d.get("label") or crit_id)
            weight = str(d.get("weight") or "medium")
            lines.append(f"#### `{crit_id}` — {label} (weight={weight})")
            lines.append("")
            if d.get("prompt"):
                lines.append(f"**Prompt:** {_md_escape(str(d['prompt']))}")
                lines.append("")
            lines.append("| Score | Meaning |")
            lines.append("| --- | --- |")
            for score_key, score_label in (("score_0", "0"), ("score_1", "1"), ("score_2", "2")):
                meaning = _md_escape(str(d.get(score_key) or "")).replace("|", "\\|")
                lines.append(f"| **{score_label}** | {meaning} |")
            lines.append("")
            lines.append(f"**Your score →** `scores.csv` row `{sample_id}` / `{crit_id}`")
            lines.append("")
    return "\n".join(lines).rstrip() + "\n"


def render_howto_grade(out_dir_name: str, n_papers: int) -> str:
    return "\n".join(
        [
            "# Blind grading pack — how to use",
            "",
            f"This pack has **{n_papers}** papers. Grade them blind (LLM grades are only in the answer key).",
            "",
            "## Files",
            "",
            "| File | Use |",
            "| --- | --- |",
            "| `index.csv` | Paper list: DOI, gene, text path, worksheet link. Put paper-level notes in `human_notes`. |",
            "| `scores.csv` | **Fill this.** One row per applicable criterion. Put `0`/`1`/`2` in `human_score`. |",
            "| `worksheets/audit_XXX.md` | Rubric guide for that paper (only criteria you score). |",
            "| `rubric_guides/` | Full host + microbe rubrics for reference. |",
            "| `llm_grades_answer_key.json` | Do not open while grading. |",
            "",
            "## Workflow",
            "",
            "1. Open `index.csv`, pick a `sample_id`.",
            "2. Open its worksheet under `worksheets/`.",
            "3. Open the paper at `text_path` and search with `gene_focus_search_terms`.",
            "4. For each criterion in the worksheet, enter **0 / 1 / 2** in `scores.csv` → `human_score`.",
            "5. Leave non-applicable criteria alone — they are not listed.",
            "",
            "After filling:",
            "",
            "```bash",
            "PYTHONPATH=src python3 pipelines/auto_lit/scripts/score_human_grading_sheet.py \\",
            f"  --pack-dir <path-to-{out_dir_name}> \\",
            "  --host-rubric $AUTO_LIT_DATA_ROOT/rubrics/host_rubric_v1.json \\",
            "  --microbe-rubric $AUTO_LIT_DATA_ROOT/rubrics/legionella_rubric.json",
            "```",
            "",
        ]
    )
...
def write_outputs(
    ...
    *,
    ...
    if per_criterion and rubric_spec is not None:
        _write_per_criterion_pack(ordered, out_dir, answer_key, rubric_spec)
    else:
        ...
        for i, rec in enumerate(ordered, start=1):
            ...
            answer_key[sample_id] = _answer_key_entry(rec)
    ...
    report_path = out_dir / "sampling_report.txt"
...
def _answer_key_entry(
    rec: PaperRecord,
    *,
    applicable: Optional[Sequence[str]] = None,
    rubric_spec: Optional[RubricSpec] = None,
) -> Dict[str, Any]:
    entry: Dict[str, Any] = {
        "doi": rec.doi,
    ...
    if applicable is not None:
        entry["applicable_criteria"] = list(applicable)
...
def _write_per_criterion_pack(
    ...
    from render_rubric_validation_guide import render_rubric_markdown  # noqa: E402

    worksheets_dir = out_dir / "worksheets"
    guides_dir = out_dir / "rubric_guides"
    worksheets_dir.mkdir(parents=True, exist_ok=True)
    guides_dir.mkdir(parents=True, exist_ok=True)

    host_guide = guides_dir / f"{rubric_spec.host_path.stem}_guide.md"
    microbe_guide = guides_dir / f"{rubric_spec.microbe_path.stem}_guide.md"
    host_guide.write_text(
        render_rubric_markdown(rubric_spec.host_rubric, source_path=rubric_spec.host_path),
        encoding="utf-8",
    )
    microbe_guide.write_text(
        render_rubric_markdown(
            rubric_spec.microbe_rubric, source_path=rubric_spec.microbe_path
        ),
        encoding="utf-8",
    )

    host_defs = _criterion_defs(rubric_spec.host_rubric)
    microbe_defs = _criterion_defs(rubric_spec.microbe_rubric)
    ...
    for i, rec in enumerate(ordered, start=1):
        ...
        rubric = rubric_spec.rubric_for_role(rec.paper_role)
        defs = (
            microbe_defs
            if (rec.paper_role or "").strip().lower() in ("query", "microbe")
            else host_defs
        )
        worksheet_rel = f"worksheets/{sample_id}.md"
        (out_dir / worksheet_rel).write_text(
            render_paper_worksheet(sample_id, rec, rubric, applicable),
            encoding="utf-8",
        )
        index_rows.append(
            {
                ...
                "text_path": rec.text_path,
                "gene_focus_search_terms": rec.gene_focus_search_terms,
                "worksheet": worksheet_rel,
                "human_notes": "",
            }
        )
        for crit_id in applicable:
            d = defs.get(crit_id) or {}
            score_rows.append(
                {
                    "sample_id": sample_id,
                    "paper_role": rec.paper_role,
        ...
        answer_key[sample_id] = _answer_key_entry(
            rec, applicable=applicable, rubric_spec=rubric_spec
        )

    index_path = out_dir / "index.csv"
    with index_path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=INDEX_CSV_FIELDS, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(index_rows)

    scores_path = out_dir / "scores.csv"
    with scores_path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=SCORE_CSV_FIELDS, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(score_rows)

    howto = out_dir / "HOW_TO_GRADE.md"
    howto.write_text(
        render_howto_grade(out_dir.name, len(ordered)),
        encoding="utf-8",
    )

    print(f"Wrote {howto}")
    print(f"Wrote {index_path}")
    print(f"Wrote {scores_path} ({len(score_rows)} criterion rows)")
    print(f"Wrote {worksheets_dir}/ ({len(ordered)} worksheets)")
    print(f"Wrote {host_guide}")
    print(f"Wrote {microbe_guide}")
...
def main() -> int:
    ...
    p.add_argument(
        "--per-criterion",
        action="store_true",
        help="Write worksheets + slim scores.csv (0-2 per applicable criterion; requires rubric paths)",
    )
    p.add_argument(
        "--per-axis",
        action="store_true",
