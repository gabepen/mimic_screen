"""Build RunAlignmentGraded POST body as plain dict (no pydantic; CPU / lit-download)."""
...
def _graded_paper_dict(
    ...
    axis_rationales = resolve_axis_rationales(
        ...
        row.get("rubric_axis_rationales")
        if isinstance(row.get("rubric_axis_rationales"), dict)
        else {},
    )
    return {
        ...
        "rubric_axis_rationales": {str(k): str(v) for k, v in axis_rationales.items()},
...
def build_run_alignment_graded_payload(
    alignment_id: str,
    ...
    *,
    ...
    papers_dir_from_graded = str(graded.get("papers_dir") or "").strip()
    if papers_dir_from_graded:
        meta = {
            "query": str(graded.get("query") or "").strip(),
            "target_id": str(graded.get("target_id") or "").strip(),
            "papers_dir": papers_dir_from_graded,
            "gene_context": graded.get("gene_context"),
            "constraints": graded.get("constraints"),
        }
    else:
    ...
    graded_papers: List[Dict[str, Any]] = []
    for row in graded.get("graded_papers") or []:
        if not isinstance(row, dict):
            continue
        graded_papers.append(_graded_paper_dict(row, host_rubric, microbe_rubric))
