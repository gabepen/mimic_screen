...
def default_host_rubric_path() -> Path:
    """Host/target rubric JSON from ``HOST_RUBRIC_PATH`` (required unless passed on CLI)."""
    path = _path_from_env("HOST_RUBRIC_PATH")
    if path is None:
        raise RuntimeError(
            "HOST_RUBRIC_PATH is not set. Export it to the host-side rubric JSON for your "
            f"dataset (files under {rubrics_dir()})."
        )
    return path


def default_microbe_rubric_path() -> Path:
    """Query/microbe rubric JSON from ``MICROBE_RUBRIC_PATH`` (required unless passed on CLI)."""
    path = _path_from_env("MICROBE_RUBRIC_PATH")
    if path is None:
        raise RuntimeError(
            "MICROBE_RUBRIC_PATH is not set. Export it to the microbe-side rubric JSON for "
            f"your dataset (files under {rubrics_dir()})."
        )
    return path
...
def resolve_rubric_paths(
    *,
    ...
    """Resolve rubric paths: explicit CLI args override env vars."""
